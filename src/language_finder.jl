...
function check_language(text::String, ngram::Integer = 3)
    best_score = 0
    lang_code = ""
    ngram_files = readdir(joinpath(Pkg.dir("LanguageFinder"),"src/Wikipedia/ngrams/" * string(ngram)))
    for i in ngram_files
        f = open(joinpath(Pkg.dir("LanguageFinder"), "src/Wikipedia/ngrams/" * string(ngram) * "/" * i, "r"))
